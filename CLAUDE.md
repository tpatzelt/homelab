...
- **djcc** — a live DJ booth (the app lives at `DJCC_SRC`, `~/coding/djcc`). Four things about it are unlike the other media stacks:
  - **Built from a checkout outside this repo.** djcc publishes no image, and its `Dockerfile` is not vendored here — it belongs beside the source it builds, so `build.context` is `${DJCC_SRC}`. Renovate cannot see a locally built image and `check.sh`'s pinning report skips anything with a `build:` key, so a `git pull` in the app repo changes nothing until `docker compose -f compose/djcc/compose.yaml build djcc` is run by hand.
